...
- Static file routes (`about.tsx`, `posts/index.tsx`) are prerendered because `announceRoutes(Router)` in `src/App.tsx` announces them to the crawl — keep that call. Dynamic routes (`posts/[slug].tsx`) are discovered by following links — a dynamic page nothing links to will not be prerendered unless added to the plugin's `pages` option in `vite.config.ts`.
