...
- **`src/App.tsrx`** — the app root as TSRX: a statement-container function body (`function App() @{ ... }`) rendering the router. The render prop is itself a statement-container arrow (`(props) => @{ ... }`) so it can use directives: `@try { ... } @pending { ... }` is the TSRX boundary syntax — it lowers to Solid's `<Loading fallback={...}>`, with the `@pending` block as the fallback shown while async reads under the route settle.
...
- **`src/routes/*.tsrx`** — route modules are TSRX too. The `fileRoutes()` scanner (`filesystem-routing` ≥ 0.3.0) statically analyzes every route module's exports to apply the page convention (default export = page, `route` export = config); for `.tsrx` routes it does that through [`@tsrx/oxc`](https://www.npmjs.com/package/@tsrx/oxc), which this template installs as a dev dependency (it is an optional peer of `filesystem-routing`, needed only when routes are `.tsrx`). `src/routes/users/[id].tsrx` shows the full shape: a `query` + `route.preload` config export next to a statement-container page component, with `@try { ... } @catch (err, reset) { ... }` around the data reads — the error boundary directive, lowered to Solid's `<Errored fallback={(err, reset) => ...}>`. An unknown user id throws from the query (branded with `markSafeError` from `@solidjs/web`, since a production SSR render replaces unbranded thrown errors with a generic "Internal Server Error" before they reach the client); the `@catch` block renders the failure and wires `reset` to a retry button. Inside `@catch`, reads of the error binding compile to accessor calls (Solid's `Errored` passes an `ErrorAccessor`), so author `err` as the plain error value — and narrow it (`err instanceof Error ? err.message : String(err)`) since thrown values are `unknown`.
